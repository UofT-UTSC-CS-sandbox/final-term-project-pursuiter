      ...
      axios
        ...
        .then((response) => {
          const filteredJobs = response.data.filter(
            (job) => job.recruiterID.toString() === user.userId,
          );
    ...
    try {
      if (editMode) {
        const response = await axios.put(
          `http://localhost:4000/jobs/${selectedJob._id}`,
          jobToSubmit,
        );
        setJobs((prevJobs) =>
          prevJobs.map((job) =>
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:4000/jobs/${jobToDelete._id}`,
      );
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        <button className="new-job-button" onClick={() => setShowJobForm(true)}>
          NEW JOB
        </button>
                ...
                <div className="job-detail-header">
                  ...
                  <div className="job-detail-actions">
                    <button
                      ...
                      onClick={() => handleSeeApplicants(selectedJob)}
                    >
                      SEE APPLICANTS
                    </button>
                    <button
