...
describe('Favorites Functionality', () => {
  describe('Recruiter Favoriting an Applicant', () => {
    it('should allow a recruiter to favorite an applicant', async () => {
      ...
      const addToFavoritesResponse = await request(app).post('/favorites/add').send({
      ...
      expect(addToFavoritesResponse.body).to.have.property('message', 'Applicant added to favorites');

      const updatedRecruiter = await request(app).get(`/favorites/${recruiter.body.userId}`);
      
      expect(updatedRecruiter.body).to.be.an('array').that.is.not.empty;
      expect(updatedRecruiter.body.some(fav => fav._id.toString() === jobSeeker.body.userId)).to.be.true;
    });
  });
  ...
  describe('Recruiter Favoriting a Job', () => {
    it('should allow a recruiter to favorite a job', async () => {
      ...
      expect(addToFavoritesResponse.status).to.equal(200);
      expect(addToFavoritesResponse.body).to.have.property('message', 'Job added to favorites');

      const updatedRecruiter = await request(app).get(`/favorites/${recruiter.body.userId}`);
      expect(updatedRecruiter.body).to.be.an('array').that.is.not.empty;
      expect(updatedRecruiter.body.some(fav => fav._id.toString() === job.body.insertedId)).to.be.true;
    });
  });
  ...
  describe('Removing Favorites', () => {
    it('should allow a recruiter to remove an applicant from favorites', async () => {
      ...
      expect(res.body).to.have.property('message', 'Applicant removed from favorites');
    ...
    it('should allow a job seeker to remove a job from favorites', async () => {
      ...
      expect(res.status).to.equal(200);
      expect(res.body).to.have.property('message', 'Job removed from favorites');

      const updatedJobSeeker = await request(app).get(`/favorites/${jobSeeker.body.userId}`);
