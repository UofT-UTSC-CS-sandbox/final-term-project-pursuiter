...
describe('Application Process', () => {
  ...
  it('should allow an applicant to apply for a job', async () => {
    ...
    const res = await request(app).post('/applications/add').send(applicationData);

    expect(res.status).to.equal(201);
    expect(res.body).to.have.property('message', 'Application added!');

    const application = await mongoose.connection.db.collection('applications').findOne({ applicantID: applicant, jobID: job });
    ...
    expect(application).to.have.property('resumeData', 'Base64EncodedResumeData');
  });
});